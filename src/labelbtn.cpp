...
void CLabelBtn::mouseReleaseEvent( QMouseEvent *pE )
{
    ...
    {
        emit onReleasedLeft( pE->position().x(), pE->position().y() );
    }
    else if( pE->button() == Qt::RightButton )
    {
        emit onReleasedRight( pE->position().x(), pE->position().y() );
    }
}
...
void CLabelBtn::mousePressEvent( QMouseEvent *pE )
{
    if( pE->button() == Qt::LeftButton )
    {
        emit onClickedLeft( pE->position().x(), pE->position().y() );
    }
    else if( pE->button() == Qt::RightButton )
    {
        emit onClickedRight( pE->position().x(), pE->position().y() );
    }
}
...
void CLabelBtn::mouseDoubleClickEvent( QMouseEvent *pE )
{
    if( pE->button() == Qt::LeftButton )
    {
        emit onDoubleClickedLeft( pE->position().x(), pE->position().y() );
    }
}
...
void CLabelBtnMove::mouseMoveEvent( QMouseEvent *pEvent ) {
    if( ( pEvent->position().x() < 0 ) || ( pEvent->position().x() >= m_Image.width()  ) )
    {
        return;
    }

    if( ( pEvent->position().y() < 0 ) || ( pEvent->position().y() >= m_Image.height() ) )
    {
        return;
    }
